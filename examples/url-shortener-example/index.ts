...
import { initYDBdriver, driver, logger, databaseName } from '../type-utils/ydb-functions';
...
const BASE_URL =
    (process.env.USE_SSL ? 'https://' : 'http://') +
...
async function run() {
    ...
    app.get(API_PREFIX + '/:shorten', async function (req, res): Promise<void> {
        if (RequestSourceUrl.isShortenCorrect(req.params.shorten || '')) {
            await driver.tableClient.withSession(async (session) => {
                const source = await selectSource(req.params.shorten, databaseName, session, logger);
    ...
    app.post(API_PREFIX, async function (req, res): Promise<void> {
        if (UrlsMatch.isSourceUrlCorrect(req.body.source || '')) {
            await driver.tableClient.withSession(async (session) => {
                const shorten = await createShorten(req.body.source, databaseName, session, logger);
